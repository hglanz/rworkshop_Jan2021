...
library(ggmap)
hdf <- get_map("boston, massachussetts")
ggmap(hdf)
```

```{r, fig.align='center', out.height = "50%", out.width = "50%", echo = F}
knitr::include_graphics("bostonuniv.PNG")
```
...
```{r, message = FALSE, fig.align='center', out.height = "50%", out.width = "50%", echo = F}
knitr::include_graphics("bostonunivzoom.PNG")
```
...
```{r, message = FALSE, fig.align='center', out.height = "50%", out.width = "50%", echo = F}
knitr::include_graphics("bostonunivsat.PNG")
```
...
* Create a `leaflet` state map of the United States with states colored and labeled according to the tuition difference variable in the colleges dataset
  
* This type of plot is a chloropleth and you should see how else you can enhance this graph!
